...
async function detectUserInstance() {
  // A instância já foi detectada no checkAuth
...
onMounted(async () => {
  ...
  if (isAuthenticated) {
    // Comentado temporariamente para focar no QR Code
    // await loadWorkflows();
    // await loadClients();
...
const filteredWorkflows = computed(() => {
    return workflows.value.filter(workflow => 
        workflow.agent_count && workflow.agent_count > 0
    );
});
...
// Função para buscar credenciais do n8n
async function loadN8nCredentials() {
    try {
        const response = await fetch('/api/n8n/credentials');
        const result = await response.json();
        if (!result.success) throw new Error(result.error);
        
        console.log('🔑 Credenciais do n8n carregadas:', result.data);
        return result.data;
    } catch (error) {
        console.error('❌ Erro ao carregar credenciais do n8n:', error);
        showNotification(`Erro ao carregar credenciais: ${error.message}`, 'error');
        return null;
    }
}
...
async function syncWorkflows() {
  ...
  try {
    await fetch('/api/sync-n8n-to-db', { method: 'POST' });
    await loadWorkflows();
...
async function saveAgentPrompt() {
    ...
    try {
        ...
        await fetch(`/api/db/agents/${editingAgentId.value}`, {
            method: 'PUT',
        ...
        const syncResponse = await fetch(`/api/db/agents/${editingAgentId.value}/sync-n8n`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
...
async function createNewAgent() {
    ...
    try {
        ...
        const response = await fetch('/api/create-workflow-with-credentials', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function saveCredentials() {
    ...
    try {
        ...
        await fetch(`/api/db/credentials/${currentWorkflow.value.id}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function updateN8nCredentials() {
    ...
    try {
        ...
        if (configCredentialsForm.googleSheets.clientId && configCredentialsForm.googleSheets.clientSecret) {
            updatePromises.push(
...
async function updateN8nCredential(type, data) {
    const response = await fetch(`/api/update-n8n-credential`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            workflowId: currentWorkflow.value.id,
...
async function testConnection(credentialType) {
    ...
    try {
        const response = await fetch(`/api/test-credential/${credentialType}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                workflowId: currentWorkflow.value.id,
...
async function deleteWorkflow(workflowId, workflowName) {
    ...
    if (!confirm(`Tem certeza que deseja deletar o workflow "${workflowName}"? Esta ação não pode ser desfeita.`)) {
        return;
    }
...
// Função para testar credenciais do n8n
async function testN8nCredentials() {
    showNotification('Testando credenciais do n8n...', 'info');
    try {
        const response = await fetch('/api/test-n8n-credentials', { method: 'POST' });
        const result = await response.json();
        if (response.ok && result.success) {
            const { data, summary } = result;
            let message = `✅ Credenciais do n8n testadas!\n`;
            message += `📊 Resumo: ${summary.totalConfigured}/${summary.totalCredentials} configuradas, ${summary.totalTested} funcionando\n`;
            
            if (data.chatwoot.configured) {
                message += `💬 Chatwoot: ${data.chatwoot.tested ? '✅ OK' : '❌ Falhou'}\n`;
            }
            if (data.gemini.configured) {
                message += `🤖 Gemini: ${data.gemini.tested ? '✅ OK' : '❌ Falhou'}\n`;
            }
            if (data.googleSheets.configured) {
                message += `📊 Google Sheets: ${data.googleSheets.tested ? '✅ OK' : '❌ Falhou'}`;
            }
            
            showNotification(message, 'success');
        } else {
            const errorMsg = result.error || 'Credenciais inválidas ou serviço indisponível';
            showNotification(`⚠️ Erro de conexão do n8n: ${errorMsg}`, 'warn');
        }
    } catch (error) {
        console.error('Erro ao testar credenciais do n8n:', error);
        showNotification(`❌ Erro ao testar credenciais do n8n: ${error.message}`, 'error');
    }
}
...
async function stopClient(clientId) {
  try {
    ...
    const response = await fetch(`/api/clients/${clientId}/stop`, { method: 'POST' });
    const result = await response.json();
...
const pageTitle = computed(() => {
  switch (currentPage.value) {
    case 'workflows': return 'Workflows & Agentes IA';
    case 'clients': return 'Clientes Multi-Tenant';
    case 'settings': return 'Configurações';
    case 'qrcode': return 'Conectar WhatsApp';
...
const pageDescription = computed(() => {
  switch (currentPage.value) {
    case 'workflows': return 'Gerencie workflows do n8n e configure agentes IA';
    case 'clients': return 'Provisione e gerencie instâncias isoladas para clientes';
    case 'settings': return 'Configurações gerais do sistema';
    case 'qrcode': return 'Conecte sua instância do WhatsApp escaneando o QR Code';
    default: return '';
  }
});
</script>
  ...
  <div class="min-h-screen surface-ground">
          ...
          <div v-if="currentPage === 'workflows'" class="grid gap-3">
            ...
            <div class="col-12 lg:col-4">
              <Card class="h-full">
                ...
                <template #content>
                  <div v-if="!workflows.length" class="text-center py-4">
                    <i class="pi pi-spin pi-spinner text-4xl text-400 mb-3"></i>
                    <p class="text-500">Carregando...</p>
                  </div>
                  ...
                  <div v-else class="flex flex-column gap-2">
                    <Card 
                      ...
                      :class="{ 'border-primary': currentWorkflow && currentWorkflow.id === workflow.id }"
                      ...
                      <template #content>
                        <div class="flex justify-content-between align-items-start">
                          <div class="flex-1">
                            ...
                            <p class="text-600 text-sm mb-2">Agentes: {{ workflow.agent_count || 0 }}</p>
                            <span 
                              class="inline-flex align-items-center gap-1 text-xs px-2 py-1 border-round"
                              :class="workflow.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'"
                            >
                              <i :class="workflow.active ? 'pi pi-check-circle text-green-600' : 'pi pi-pause-circle text-gray-500'"></i>
            ...
            <div class="col-12 lg:col-8">
              <div v-if="!currentWorkflow" class="flex flex-column align-items-center justify-content-center" style="min-height: 400px;">
              ...
              <div v-else class="flex flex-column gap-3">
                  ...
                  <template #content>
                    <div class="flex justify-content-between align-items-center p-2">
                      <div>
                        ...
                        >
                          <i :class="currentWorkflow.active ? 'pi pi-check-circle' : 'pi pi-pause-circle'"></i>
                ...
                <Panel header="Agentes" toggleable class="compact-panel">
                  ...
                  <div class="flex gap-3 flex-nowrap">
                    <div v-for="agent in currentWorkflow.agents" :key="agent.id" class="flex-shrink-0" style="width: 200px;">
                      <div 
                        class="agent-card cursor-pointer"
                        @click="startEditPrompt(agent)"
                      >
                          ...
                          <div class="prompt-section">
                            ...
                            <div class="prompt-content">
                              ...
                              <p v-else class="prompt-empty">
                                Clique para adicionar um prompt...
                              </p>
                            </div>
                            
                            <div class="prompt-action">
                              <small v-if="agent.prompt && agent.prompt.length > 100">
                      ...
                      >
                        <template #content>
                          <div class="text-center p-2">
                            ...
                            <span 
                              class="inline-flex align-items-center gap-1 text-xs px-2 py-1 border-round"
                              :class="chatwootConfigured ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'"
                            >
                              <i :class="chatwootConfigured ? 'pi pi-check-circle' : 'pi pi-exclamation-triangle'"></i>
                      ...
                      >
                        <template #content>
                          <div class="text-center p-2">
                            ...
                            <span 
                              class="inline-flex align-items-center gap-1 text-xs px-2 py-1 border-round"
                              :class="geminiConfigured ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'"
                            >
                              <i :class="geminiConfigured ? 'pi pi-check-circle' : 'pi pi-exclamation-triangle'"></i>
                      ...
                      >
                        <template #content>
                          <div class="text-center p-2">
                            ...
                            <span 
                              class="inline-flex align-items-center gap-1 text-xs px-2 py-1 border-round"
                              :class="googleSheetsConfigured ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'"
                            >
                              <i :class="googleSheetsConfigured ? 'pi pi-check-circle' : 'pi pi-exclamation-triangle'"></i>
              ...
              <Card>
                ...
                <template #content>
                  <div v-if="!clients.length" class="text-center py-8">
                    ...
                    <p class="text-500 mb-4">Crie sua primeira instância multi-tenant para começar</p>
                    <Button 
                        ...
                        <template #content>
                          <div class="flex flex-column h-full">
                            <div class="flex justify-content-between align-items-start mb-3">
                              <div class="flex-1">
                                <h3 class="font-semibold text-900 mb-1">{{ client.client_name }}</h3>
                                <p class="text-600 text-sm mb-2">{{ client.client_email }}</p>
                              </div>
                              <span 
                                class="inline-flex align-items-center gap-1 text-xs px-2 py-1 border-round"
                                :class="client.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'"
                              >
                                <i :class="client.status === 'active' ? 'pi pi-check-circle' : 'pi pi-pause-circle'"></i>
                            ...
                            <div class="flex flex-column gap-2 mb-3">
                              ...
                              <div class="flex align-items-center gap-2">
                                ...
                                <span class="text-xs text-600">n8n: :{{ client.n8n_port || (client.port + 1000) }}</span>
                              </div>
                            </div>
                            ...
                            <div class="flex flex-column gap-2 mt-auto">
                              <div class="grid gap-2">
                                ...
                                <div class="col">
                                  <a 
                                    :href="`http://localhost:${client.n8n_port || (client.port + 1000)}`" 
                                    target="_blank"
                                    class="no-underline"
                                  >
                                    <Button 
                        ...
                        <div class="col-12 sm:col-6 md:col-3">
                          <Card class="border-1 surface-border text-center">
                            <template #content>
                              ...
                              <h4 class="text-xl font-bold text-900 mb-1">{{ clients.length * 2 }}</h4>
                        ...
                        <div class="col-12 sm:col-6 md:col-3">
                          <Card class="border-1 surface-border text-center">
                            <template #content>
                              ...
                              <h4 class="text-xl font-bold text-900 mb-1">{{ filteredWorkflows.length }}</h4>
              ...
              <Card>
                ...
                <template #content>
                  <div class="flex flex-column gap-6">
                    ...
                    <!-- Status da Conexão -->
                    <div class="text-center">
                      <div v-if="whatsappConnection.loading" class="flex flex-column align-items-center gap-3">
                        <i class="pi pi-spin pi-spinner text-4xl text-primary"></i>
                        <p class="text-600">Verificando status da conexão...</p>
                      </div>
                      
                      <div v-else-if="whatsappConnection.connected" class="flex flex-column align-items-center gap-3">
                        ...
                        <p class="text-600 mb-4">Sua instância está ativa e funcionando perfeitamente.</p>
                        <div class="flex gap-2">
                          <Button 
                      ...
                      <div v-else class="flex flex-column align-items-center gap-4">
                        <div class="w-6rem h-6rem border-circle bg-orange-100 flex align-items-center justify-content-center">
                          <i class="pi pi-whatsapp text-4xl text-orange-600"></i>
                        </div>
                        <h3 class="text-2xl font-semibold text-900 m-0">Conectar WhatsApp</h3>
                        <p class="text-600 mb-4">Escaneie o QR Code com seu WhatsApp para conectar sua instância.</p>
                        
                        <!-- QR Code -->
                        <div v-if="whatsappConnection.qrCode" class="flex flex-column align-items-center gap-3">
                          <Card class="border-2 border-dashed surface-border">
                            <template #content>
                              <div class="text-center p-4">
                                <img 
                                  :src="`data:image/png;base64,${whatsappConnection.qrCode}`" 
                          ...
                          <div class="text-center">
                            ...
                            <p class="text-sm text-600 mb-2">2. Toque em ⋮ → Dispositivos conectados</p>
                            <p class="text-sm text-600 mb-4">3. Toque em "Conectar um dispositivo" e escaneie o código</p>
                            <Button 
                        ...
                        <Card class="border-1 surface-border h-full">
                          <template #content>
                            <div class="text-center">
                              ...
                              <p class="text-600 text-sm">Suas mensagens ficam criptografadas end-to-end.</p>
                            </div>
                          </template>
                        </Card>
                      </div>
                        ...
                        <Card class="border-1 surface-border h-full">
                          <template #content>
                            <div class="text-center">
                              ...
                              <p class="text-600 text-sm">Conecte em segundos e comece a usar imediatamente.</p>
                            </div>
                          </template>
                        </Card>
                      </div>
                        ...
                        <Card class="border-1 surface-border h-full">
                          <template #content>
                            <div class="text-center">
                              ...
                              <p class="text-600 text-sm">Mensagens sincronizadas automaticamente.</p>
                            </div>
                          </template>
                        </Card>
                      </div>
    ...
    >
      <div class="flex flex-column gap-4">
        ...
        <div class="grid">
          ...
          <div class="col-12 md:col-6">
            <FloatLabel>
              <InputText id="chatwootToken" v-model="newAgentForm.chatwootAccessToken" class="w-full" />
          ...
          <div class="col-12 md:col-6">
            <FloatLabel>
              <InputText id="googleClientSecret" v-model="newAgentForm.googleClientSecret" class="w-full" />
    ...
    >
      ...
      <div v-if="editingCredentialType === 'googleSheets'" class="flex flex-column gap-4">
        ...
        <div class="p-3 border-1 surface-border border-round">
          ...
          <code class="text-xs text-500 block mt-2">https://auto.criard.me/rest/oauth2-credential/callback</code>
        </div>
      </div>
    ...
    >
      <div class="flex flex-column gap-4">
        ...
        <p class="text-600 text-sm mb-3">
          Configure o prompt que será usado por este agente para interagir com os usuários. 
          Seja específico e claro sobre o comportamento esperado.
        </p>
    ...
    >
      <div class="flex flex-column gap-4">
        ...
        <p class="text-600 text-sm mb-3">
          Crie uma nova instância isolada do Chatwoot + n8n para este cliente. 
          Serão geradas URLs únicas e ambientes completamente separados.
        </p>
      ...
      <template #footer>
        <div class="flex justify-content-end gap-2">
          ...
          <Button 
            ...
            icon="pi pi-plus" 
            @click="createClient"
            :disabled="!newClientForm.clientName || !newClientForm.clientEmail || isProvisioningClient"
